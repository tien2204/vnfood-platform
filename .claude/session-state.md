...
## Trạng thái hiện tại
**Cập nhật lần cuối:** 2026-06-05 (RBAC SP1 role foundation XONG; đang làm RBAC 3 tầng)
**Branch:** `feat/canonical-recipes` (push lại — local ahead ~54 commit)
**Task đang làm:** **RBAC 3 tầng (user/cộng tác viên/admin)** — tách 5 sub-project build tuần tự. **SP1 (role foundation) XONG**: `app/core/roles.py` (USER/COLLABORATOR/ADMIN + `role_at_least` hierarchy admin⊇collab⊇user), `require_collaborator` guard (deps.py), admin gán role `collaborator` (admin.py validate `roles.ROLES`). Không migration. **Tiếp: SP2** (pipeline duyệt 2 tầng user→CTV→admin + "my recipes" private + source attribution). SP3 = claim-lock đơn giản (bỏ co-review), SP4 variant-from-saved, SP5 portal CTV/admin. Spec/plan `docs/superpowers/{specs,plans}/2026-06-05-rbac-sp1*`.
Hạ tầng có sẵn (tái dùng cho SP2+): recipe `status` pending/approved/rejected + `author_id`/`original_author_name`/`source`/`variant_label`, non-approved chỉ author/admin thấy; admin router đầy đủ (stats/charts/recipe moderate/user role/comments/ingredients); `UserOut.role` đã expose.
---
