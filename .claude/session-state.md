...
## Trạng thái hiện tại
**Cập nhật lần cuối:** 2026-06-17 (Camera capture /recognize + P1 overview món đa biến thể + P2 scope /recipes về 103 + P3 mô tả vị giác — 4 mảng subagent-driven, Ready to merge, CHƯA merge main)
**Branch:** `feat/monngonmoingay-restyle`
**Task đang làm:** 4 feature xong code trên branch (đã push, KHÔNG merge main): (1) **Camera** chụp ảnh /recognize; (2) **P1** thẻ "Giới thiệu món" cho 57 món đa biến thể; (3) **P2** /recipes chỉ còn 103 món + biến thể (~529); (4) **P3** mô tả vị giác `flavor_text` cho 529 recipe. Migration **0017** (`ai_class_slug`) + **0018** (`flavor_text`) đã apply (dev). Chi tiết entry 2026-06-17 ngay dưới.
**⚠ Cần làm thủ công:** restart `uvicorn` (load cột mới) + `npm run dev`; **QA thủ công** cả 4 mảng (camera cần HTTPS/localhost; P1 overview card; P2 chip group + đếm ~529; P3 mô tả ở recipe detail). Migration đã apply ở dev — môi trường mới chạy `alembic upgrade head`.

### ✅ Camera + P1 overview + P2 scope-103 + P3 flavor_text — 2026-06-17 (subagent-driven, ~30 commit)
Spec/plan: `docs/superpowers/{specs,plans}/2026-06-16-recognize-camera-capture*`, `2026-06-17-multi-variant-dish-overview*`, `2026-06-17-scope-recipes-to-103*`, `2026-06-17-recipe-flavor-text*`.
- **Camera chụp ảnh (/recognize):** NEW `components/ai/CameraCapture.tsx` (modal full-screen `getUserMedia` → canvas → File, đổi camera trước/sau, review "Chụp lại/Dùng ảnh", cleanup stream+blob URL, render-driven attach tránh race, guard toBlob sau close). `ImageDropzone.tsx`: 2 card cạnh nhau (Kéo-thả | Chụp ảnh). 0 thay đổi backend (file qua đúng `handleSelect`).
- **P1 — overview món đa biến thể:** `core/variant_config.py` += `MULTI_VARIANT_SLUGS` (57 slug, 2 nhóm rich+moderate) + `is_multi_variant()`. NEW `ai/dish_overviews.json` (**seed LLM 57/57**: display_name/tasting/key_ingredients/main_steps/variant_examples) + `services/dish_overview_service.py` (loader chống JSON hỏng) + `scripts/seed_dish_overviews.py` (resume-safe, utf-8 console). `ai_service.recognize_image` trả thêm `is_multi_variant`+`dish_overview`. Frontend: NEW `DishOverviewCard.tsx`, `VariantBadge` (CanonicalBadge.tsx), `RecognitionResult` hiện overview phía trên + đổi tiêu đề "Một công thức tiêu biểu" + badge "1 trong nhiều biến thể" (giữ thẻ recipe — phương án b). Món đơn nhất giữ nguyên.
- **P2 — scope /recipes về 103 + biến thể:** Migration **0017** `recipes.ai_class_slug VARCHAR(80) NULL` + index. `class_names.py` += `resolve_ai_class(title,slug)` (exact-slug → title-prefix phân biệt dấu longest-match → None), `GROUP_OF_SLUG`/`slugs_for_group`/`VALID_GROUPS`. `scripts/tag_ai_class_slug.py` backfill (**6409 tagged / 529 canonical in-scope**, 0 false positive). `recipe_service` 3 filter (list/search/featured) + `ai_service._in_recipes_page_pool`: pool `(is_canonical AND ai_class_slug NOT NULL) OR source=user`. `list_recipes` += param `group` → `ai_class_slug IN slugs_for_group(group)`; `GET /recipes?group=`. Frontend `RecipeBrowse.tsx`: chip+dropdown đổi `keyword`→`group` (8 chip = 8 GROUP_CLASSES); nhãn "Món Khô"→**"Món mặn"** (vì nhóm MON_KHO_NUONG gồm cả xào/chiên, không thuần khô — taxonomy AI là source of truth). **2.806→~529.**
- **P3 — mô tả vị giác:** Migration **0018** `recipes.flavor_text TEXT NULL`. `scripts/seed_flavor_text.py` (LLM judge-and-rewrite, gpt-4o-mini, JSON mode, batch commit). **Seed 529/529** (LLM viết lại HẾT, không giữ cái nào — user CHẤP NHẬN vì chất lượng tốt + `description` gốc còn nguyên trong DB). Display `flavor_text or description` ở `get_recipe_detail` + `_build_dish_recipe_from_canonical`. Final review fix: change-request edit clear `flavor_text` (tránh che edit); FTS index thêm `flavor_text` (3 tsvector). Pytest 26 pass. KHÔNG đổi schema/frontend.
- **Decisions:** P1 trigger = config list (không data-driven); overview content = JSON seed LLM sửa tay; P2 scope = title-prefix (521→thực 529, không exact-slug 294); P3 lưu = cột mới (không ghi đè) + judge-and-rewrite; recognize suggestions align pool 529. Seeder Windows: `sys.stdout.reconfigure(encoding="utf-8")` (cp1252).
- **Pre-existing chưa fix (ngoài scope):** `_in_recipes_page_pool` (ai_service) không có `is_dessert=False` như 3 filter browse → gợi ý recognize có thể lọt món dessert-tagged.

### ✅ Auth fix + gỡ Follow/Feed + gỡ Gợi-ý-nguyên-liệu + bỏ "Our Menu" — 2026-06-13/14
- **Fix bug auth "mở tab 2 bắt login lại":** single-flight refresh. NEW `lib/single-flight.ts`, `lib/auth-session.ts` (`isSessionDead`: chỉ logout khi no-refresh hoặc 401/403), `lib/jwt.ts` (decode thuần), `lib/auth-refresh.ts`. `api.ts` interceptor dùng `refreshAccessToken()`, chỉ logout nếu `isSessionDead` → soft redirect `/auth/login?next=`. NEW `components/layout/SessionKeepAlive.tsx` (refresh chủ động trước hết hạn, mount trong ConditionalLayout). Backend: `auth_service.refresh_access_token` + `/auth/refresh` trả **cả access+refresh** (sliding session). Test `lib/auth-fix.test.ts`.
- **Gỡ Follow/Feed hoàn toàn:** XÓA `api/v1/feed.py`, `app/feed/`, `components/users/{FollowButton,UserCard,UserStatsBar}.tsx`. Migration **`0016_drop_follows.py`** (down_revision `0306fd718821`) DROP bảng `follows` CASCADE. Sửa main.py / users.py / social_service / user_service / schemas(user,recipe) / recipe_service / admin_service / models(social bỏ class `Follow`, user, `__init__`). Cộng đồng giờ chỉ còn rating/comment/save. ⚠ User phải chạy `alembic upgrade head`.
- **Gỡ hẳn "Gợi ý món từ nguyên liệu" (/suggest):** XÓA `api/v1/ingredients.py` (router /popular,/search,/suggest-recipes), `services/ingredient_service.py`, `services/ingredient_ai_service.py`, `app/suggest/`, `components/ingredients/` (6 component), block types "Ingredient Suggest" trong `types.ts`; bỏ include router trong main.py. **KHÔNG migration** (không có bảng riêng — query thẳng recipe_ingredients). GIỮ: recipe `Ingredient` model, `staff/ingredients` (/admin/ingredients merge/rename), `recommend_service` (/meal-plans/suggestions — khác feature). Backend import OK, tsc 0 lỗi. (Lượt #4 trước chỉ ẩn facet + để /suggest dormant; lượt này xóa hẳn.)
- **Bỏ section "Our Menu" homepage:** xóa khối `<section>` "Our Menu" trong `app/page.tsx` + dead code kéo theo (`menuPreview`, `MenuListItem`, `CenteredSectionHeader`, `stripEmoji`, `getRecipeImageUrl` + import `ChefHat/Clock/Users/RecipeImage`). tsc 0 lỗi.
- **(Song song) Quyển ĐATN LaTeX `DATN_REPORT/`:** viết xong cả 6 chương + bibliography (bibtex, KHÔNG biber vì path "Đ"). Còn render PlantUML + chụp screenshot. Chi tiết: memory `project-datn-latex`.
...
- **Backend (ít):** `RecipeCardWithStatus += claimed_by` (id) + populate trong `_build_recipe_card_with_status`/`list_review_queue`; `ChangeRequestOut += payload` (cho admin review nội dung đề xuất). **Admin account CRUD** (`admin.py`+`admin_service.py`): `POST /admin/users` (sinh temp password 1 lần via `security.generate_temp_password`, `hash_password`), `PATCH /admin/users/{id}` (full_name/email, 409 email trùng), `POST /admin/users/{id}/reset-password` (temp mới 1 lần), `DELETE /admin/users/{id}` (raw SQL delete — dựa FK ondelete DB: recipes/ai_logs/ratings/comments SET NULL, saved/meal_plans/follows/change_requests CASCADE; self-delete 400). KHÔNG migration.
- **Frontend:** Di chuyển `app/admin/*` → `app/staff/*` (5 trang cũ: dashboard/users/recipes/comments/ingredients) + `AdminLayout`→`components/staff/StaffLayout.tsx` **role-aware** (nav lọc theo role; title "Cộng tác viên"/"Quản trị"). `middleware.ts`: `/staff`→collaborator+, subtree admin-only (`STAFF_ADMIN_RE`)→admin, `/admin*`→`/staff*` redirect. `ConditionalLayout` đổi prefix `/staff`. **ContextSwitcher** (Navbar + staff header): Người dùng↔(CTV|Quản trị), ẩn với role user. **Màn CTV:** `/staff/review` (queue claim/nhả/khóa, phân biệt mine vs other qua `claimed_by===user.id`), `/staff/review/[id]` (review detail HỢP NHẤT 2 tầng theo status×role: pending_collaborator→duyệt/từ chối CTV; pending_admin+admin→đăng/từ chối; khác→read-only), `/staff/proposals` (reuse list change-request; `/me/change-requests`→redirect). **Màn admin:** `/staff/admin-review` (queue pending_admin→mở review detail), `/staff/change-requests` (duyệt/từ chối CR, xem payload), `/staff/users/new` (tạo TK + hiện temp pw 1 lần copyable) + nút "Tạo tài khoản" + detail thêm Sửa/Đặt lại mật khẩu/Xóa (ẩn Xóa khi tự xem). Shared `components/recipes/RecipeContent.tsx` (render read-only nguyên liệu/bước). Fix 5 lỗi tsc tiền-tồn (me/profile, recognize, dashboard recharts) để build xanh.
- **Manual checklist (chưa chạy — cần user):** CTV: switcher→/staff/review→claim→mở→duyệt; nhả; recipe người khác claim hiện 🔒; vào /staff/users bị đá về /staff/review. Admin: /staff/admin-review→đăng; /staff/change-requests→duyệt(áp dụng)/từ chối; AI-class delete CR hiện toast guard; /staff/users→tạo(temp pw)→sửa→reset→xóa; không xóa được chính mình; /admin redirect /staff.
...
Spec/plan `docs/superpowers/{specs,plans}/2026-06-05-rbac-sp2*`. 4 task subagent-driven, final review "ready to merge" (smoke 11/11 + 7/7).
- **State machine** `Recipe.status` 5 trạng thái: `private`→`pending_collaborator`→`pending_admin`→`approved`/`rejected` (String, KHÔNG migration; legacy `pending`→`pending_collaborator`). `recipe_service`: `create`→private; `submit_recipe`/`withdraw_recipe` (owner 403), `collaborator_approve`/`collaborator_reject`, `admin_publish` (set source=user + `original_author_name`)/`admin_reject` — mỗi hàm `_assert_status` (409 sai tầng). Sửa recipe đã `approved`/`pending_admin` → quay lại `pending_collaborator` re-review.
- **Routes** `recipes.py`: `POST /{id}/submit|withdraw` (owner), `/{id}/review/approve|reject` (require_collaborator), `/{id}/publish|admin-reject` (require_admin), `GET /review/queue/collaborator|admin`. **"My recipes" tái dùng `GET /users/me/recipes` có sẵn** (đã bỏ duplicate MyRecipeCardOut/list_my_recipes — dùng `RecipeCardWithStatus`/`get_my_recipes`). Mới chỉ `list_review_queue`.
- **Visibility**: browse 3 hàm (list/search/featured) `is_canonical OR source='user'` → recipe cộng đồng lên browse (Cookpad pool vẫn loại; related/variants vẫn canonical-only). Detail non-approved: owner/CTV/admin xem được.
- **Frontend** (enhance page có sẵn): `/me/recipes` thêm nút Gửi duyệt (private/rejected)/Thu hồi (pending_collaborator) + tab 5 trạng thái; `StatusBadge` 5 trạng thái; `RecipeCard` badge "Cộng đồng" (approved user recipe).
- **Reviewer screens (queue UI CTV/admin) = SP5** (endpoint đã có, UI sau). **Follow-up:** TS `User.role` còn `"user"|"admin"` → thêm `collaborator` ở SP5.
Hạ tầng có sẵn (tái dùng cho SP2b+): admin router đầy đủ (stats/charts/recipe moderate/user role/comments/ingredients); `get_my_recipes`/`RecipeCardWithStatus`; `reject_reason`.
---
### (lịch sử) Facet filter v2 (parity 6 hạng mục MNMN) XONG — 6 task subagent-driven, reviewed, final review "ready to merge". Thêm 2 facet (Nguyên liệu, Cách nấu) → đủ 6 hạng mục như monngonmoingay.com; UI dropdown grouped-checkbox + nút "Lọc thông tin"; label tiếng Việt chuẩn. Tất cả 2615 canonical NULL=0 cả 6 facet. Cần restart uvicorn + `npm run dev`. Trước đó: **Sub-project 3/3 (facet filter v1) XONG** — 6 task subagent-driven, reviewed, final review "ready to merge". Cần restart uvicorn + `npm run dev` để thấy chip facet + recognize fix mới. Trước đó: MNMN crawl→canonical FULL đã chạy: **2615 canonical** (verbatim+video+metadata), harness PASS, đã fix 4 dup-title. **AI recognize→/recipes link verified 103/103** (17 món AI giờ trỏ canonical MNMN). Cần restart uvicorn để API trả data mới. Còn: thesis muc-luc cập nhật (cooking-mode/voice usecase + sửa "30 món"→103); 2 sub-project chưa làm: personalization (embedding), substitution (curated+LLM). Chi tiết cuối file.
