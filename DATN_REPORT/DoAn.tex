...
\def \TITLE{Xây dựng website nhận diện ảnh món ăn và tư vấn nấu món ăn}
\def \AUTHOR{Vũ Hữu Tiến}
...
% Hình biểu đồ "chịu thiếu ảnh": nếu figures/<tên>.png chưa có (chưa render từ
% diagrams/<tên>.puml bằng PlantUML) thì hiện khung placeholder để vẫn
% compile được; có ảnh là tự thay. #1=tên file (không đuôi), #2=caption, #3=label.
\newcommand{\diagramfig}[3]{%
  \begin{figure}[H]
    \centering
    \IfFileExists{figures/#1.png}{%
      \includegraphics[width=0.92\textwidth,height=0.8\textheight,keepaspectratio]{#1}%
    }{%
      \fbox{\parbox[c][4.2cm][c]{0.9\textwidth}{\centering\itshape
        [Chưa có ảnh] #2.\\[6pt]
        Render PNG từ \detokenize{diagrams/#1.puml} (PlantUML) và lưu vào \detokenize{figures/#1.png}.}}%
    }
    \caption{#2}
    \label{#3}
  \end{figure}%
...
% Ảnh chụp màn hình: thiếu figures/<tên>.png thì hiện placeholder, có thì tự chèn.
\newcommand{\screenfig}[3]{%
  \begin{figure}[H]
    \centering
    \IfFileExists{figures/#1.png}{%
      \includegraphics[width=0.85\textwidth,height=0.78\textheight,keepaspectratio]{#1}%
    }{%
      \fbox{\parbox[c][4cm][c]{0.9\textwidth}{\centering\itshape
        [Chưa có ảnh chụp] #2.\\[6pt]
        Chụp màn hình chức năng và lưu vào \detokenize{figures/#1.png}.}}%
    }
    \caption{#2}
    \label{#3}
  \end{figure}%
...
% =========================== BODY ===============
\begin{document}
% Không giãn keo dọc để lấp full trang -> tránh khoảng trắng lớn giữa tiêu đề và
% bảng/hình ở các trang ít nội dung; phần dư dồn xuống đáy trang.
\raggedbottom
% \newgeometry{top=2cm, bottom=2cm, left=2cm, right=2cm}
\subfile{Bia} % Phần bìa
% \subfile{Bia_lot} % Bỏ bớt một bìa trùng (bìa lót trùng nội dung với bìa ngoài)
% \restoregeometry
...
% Bỏ Tóm tắt tiếng Anh theo yêu cầu
% \newpage
% \pagenumbering{gobble}
...
% --- Bỏ mục hướng dẫn mẫu của template (kéo theo citation rác [10]-[16]) ---
%\newpage
%\pagestyle{fancy} % Áp dụng header và footer
