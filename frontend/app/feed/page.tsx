...
export default function FeedPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-6 pb-24 lg:pb-6">
      ...
      {/* Discover mode CTA */}
      {isDiscoverMode && (
        <div className="bg-[var(--color-brand-pink-bg)] border border-primary/20 rounded-2xl p-4 mb-6 flex items-start gap-3">
          <UserPlus className="w-5 h-5 text-primary shrink-0 mt-0.5" />
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-foreground">Chưa theo dõi ai</p>
            <p className="text-xs text-muted-foreground mt-0.5">
              Theo dõi các đầu bếp để xem công thức mới nhất của họ trong Bảng tin
            </p>
          </div>
          <Link href="/recipes">
            <Button size="sm" variant="outline" className="border-primary text-primary hover:bg-primary hover:text-white shrink-0 text-xs">
              Khám phá
            </Button>
          </Link>
        </div>
...
function FeedCard({ item }: { item: FeedItem }) {
  ...
  return (
    <article className="bg-card rounded-2xl border border-border overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-muted">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-baseline gap-1.5 flex-wrap">
            ...
            <span className="text-xs text-muted-foreground">đã đăng công thức mới</span>
          </div>
          <p className="text-xs text-muted-foreground mt-0.5">{relativeTime(posted_at)}</p>
        </div>
      </div>
      ...
      {/* Footer */}
      <div className="px-4 pb-3 flex items-center justify-between border-t border-muted pt-3">
        <Link href={`/recipes/${recipe.id}`}>
          <Button size="sm" variant="outline" className="text-xs border-border text-muted-foreground hover:border-primary hover:text-primary">
            Xem công thức
          </Button>
        </Link>
        <SaveButton
