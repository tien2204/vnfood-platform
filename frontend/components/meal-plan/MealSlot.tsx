...
export default function MealSlot({ mealType, items, disabled = false, onAdd, onDelete, onUpdateServings }: MealSlotProps) {
  ...
  return (
    <div className={`rounded-xl border overflow-hidden flex flex-col ${MEAL_COLORS[mealType]}`}>
      ...
      {items.length === 0 ? (
        disabled ? (
          <div className="flex-1 flex items-center justify-center text-muted-foreground text-xs py-2">
            Không có món
          </div>
        ) : (
          <button
              ...
              {expandedItem === item.item_id && (
                <div className="px-1.5 pb-1.5 border-t border-border">
                  <div className="flex items-center gap-1.5 pt-1.5">
                    ...
                    {disabled ? (
                      <span className="text-xs font-semibold">{item.servings} người</span>
                    ) : (
                      <>
                        <button
