  ...
  return (
    <div className={`rounded-lg border p-2 min-h-[80px] flex flex-col gap-1.5 ${MEAL_COLORS[mealType]}`}>
      <div className="flex items-center justify-between mb-0.5">
        <span className="text-xs font-semibold text-[#7C6A56] uppercase tracking-wide">
          {MEAL_LABELS[mealType]}
        </span>
        <button
          onClick={onAdd}
          className="w-5 h-5 rounded-full bg-white border border-[#E8DDD4] flex items-center justify-center hover:bg-[#E85D26] hover:border-[#E85D26] hover:text-white transition-colors"
          title="Thêm món"
        >
      ...
      {items.length === 0 ? (
        ...
        <div className="flex flex-col gap-1">
          {items.map((item) => (
            <div key={item.item_id} className="bg-white rounded border border-[#E8DDD4] overflow-hidden">
              <div className="flex items-center gap-1.5 p-1.5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-[#7C6A56]">{item.servings} người</p>
                </div>
                <div className="flex items-center gap-0.5 shrink-0">
                  <button
                    onClick={() => setExpandedItem(expandedItem === item.item_id ? null : item.item_id)}
                    className="p-0.5 hover:text-[#E85D26] text-[#7C6A56] transition-colors"
                  >
                    {expandedItem === item.item_id ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
                  </button>
                  <button
                    onClick={() => onDelete(item.item_id)}
                    className="p-0.5 hover:text-red-500 text-[#7C6A56] transition-colors"
                  >
                    <X className="w-3 h-3" />
                  </button>
                </div>
              ...
              {expandedItem === item.item_id && (
                <div className="px-1.5 pb-1.5 border-t border-[#F0E8E0]">
                  <div className="flex items-center gap-1.5 pt-1.5">
                    <span className="text-[10px] text-[#7C6A56]">Khẩu phần:</span>
                    <button
                      onClick={() => onUpdateServings(item.item_id, Math.max(1, item.servings - 1))}
                      className="w-4 h-4 rounded border border-[#E8DDD4] flex items-center justify-center text-xs hover:bg-[#F7F0E8]"
                    >
                      −
                    </button>
                  ...
                  {item.note && (
                    <p className="text-[10px] text-[#7C6A56] mt-1 italic">{item.note}</p>
                  )}
                </div>
              )}
            </div>
