...
export function VariantsAccordion({ variants, title }: { variants: RecipeMini[]; title?: string }) {
  ...
  return (
    <section className="mt-6 border-t border-[#E8DDD4] pt-6">
      <button
          ...
          {variants.map((v) => {
            ...
            return (
              <Link
                key={v.id}
                href={`/recipes/${v.id}`}
                className="block rounded-md border border-[#E8DDD4] bg-white overflow-hidden hover:shadow transition-shadow"
              >
                ...
                <div className="p-2">
                  ...
                  {v.variant_label && (
                    <p className="text-xs text-[#7C6A56] mt-0.5">{v.variant_label}</p>
                  )}
                </div>
              </Link>
            );
