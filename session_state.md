...
## Trạng thái hiện tại
**Cập nhật lần cuối:** 2025-05-10  
**Branch:** `main`  
**Task đang làm:** Week 3 — AI Features (Prompt 11 xong, chuẩn bị Prompt 12)
...
### 🔄 Week 3 — AI Features (Đang làm — Prompt 11 xong)
- [x] AI foundation: VNFoodPredictor load models + predict() test được
- [ ] Prompt 12: POST /api/v1/ai/recognize endpoint + OpenAI fallback
...
**Prompt 12 — POST /api/v1/ai/recognize**
- Nhận file ảnh (multipart), validate size + min dimension
- Gọi `predictor.predict()` → nếu `needs_fallback` → gọi OpenAI Vision fallback
- Query DB tìm recipes theo `predicted_class` / display_name
- Trả về: predicted_class, display_name, confidence, model_used, suggested_recipes (top 6)
- File: `backend/app/services/ai_service.py` (OpenAI fallback) + thêm endpoint vào `backend/app/api/v1/ai.py`
